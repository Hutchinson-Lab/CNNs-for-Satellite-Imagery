# Run.py -- start here for Convolutional Neural Networks for Satellite Imagery:
...
if pre['calc_channel_means'] or pre['calc_channel_means_stdDevs']:
	...
	dataloader = satellite_dataloader(img_type, paths['img_dir'], labels_path, split=split,
									  size=img_size, img_ext=img_ext, bands=bands, batch_size=1,
...
if config['model']['arch'] == 'resnet18':
	if task == 'eurosat':
		from src.resnet_eurosat import ResNet
		CNN = ResNet(num_classes=data['num_classes'], in_channels=bands,
					 num_blocks=blocks, pretrained=config['model']['pretrained'])
	else:
		from src.resnet import ResNet
		CNN = ResNet(num_classes=data['num_classes'], num_blocks=blocks,
...
if training['train']:
	# freeze all but the last FC layer
	if config['model']['freeze_conv']:
	...
	if config['model']['freeze_conv']:
		optimizer = optim.AdamW(CNN.fc.parameters(), lr=lr, betas=(beta1, 0.999), weight_decay=weight_decay)
	else:
